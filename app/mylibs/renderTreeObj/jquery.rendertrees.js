...
let renderTrees = {
    ...
    blockChanges: {
        ...
        open () {
            ...
            // Переменные элемента
            let top = (currentElem.offset().top - $(`#tree_${renderTrees.index + 1}`).offset().top) + (currentElem.height() - this.elem[renderTrees.index].height())/2;
            let left = currentElem.offset().left;
