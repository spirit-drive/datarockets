...
let renderTrees = {
    ...
    line: {
        ...
        init () {
            let pendingClick = 0;
            // Если еще не определен index то берем номер количества вызовов renderTrees.start
                ...
                switch(e.originalEvent.detail){
                    ...
                    case 1:
                        pendingClick = setTimeout(() => {
                            ...
                            if (renderTrees.blockChanges.isNotOpen()){
                                renderTrees.line.hideShowSublist();
                            }
                        },200);
                        break;
    ...
    blockChanges: {
        ...
        init () {
            ...
            // Кнопки, элемент и инпут не требуют снятия предыдущих обработчиков событий, т.к. инициализация происходит только при удалении всего дерева и создания нового, но во время этого процесса и блок изменений создается новый
